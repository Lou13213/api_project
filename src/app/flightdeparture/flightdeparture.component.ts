...
export class FlightdepartureComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.searchCtrlDestination.valueChanges
      .pipe(
        switchMap((value: string) =>
          this.dataservice.GetFlightsToDestination(value)
        )
      )
    ...
    this.searchCtrlFlightnumber.valueChanges
      .pipe(
        switchMap((value: string) =>
          this.dataservice.GetFlightsToFlightnumber(value)
        )
      )
      .subscribe((data: Flight[]) => (this.flights = data));
    ...
    this.searchCtrlAirlines.valueChanges
      .pipe(
        switchMap((value: string) =>
          this.dataservice.GetFlightsToAirlines(value)
        )
      )
      .subscribe((data: Flight[]) => (this.flights = data));
  }
